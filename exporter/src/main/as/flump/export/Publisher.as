...
public class Publisher
{
    ...
    public function Publisher (exportDir :File, project :ProjectConf, projectName :String) {
        ...
        for each (var exportConf :ExportConf in project.exports) {
            _confs.push(exportConf);
        }
    }
    ...
    public function modified (libs :Vector.<XflLibrary>, idx :int = -1) :Boolean {
        ...
        return instantiate(libs, idx).some(function (exportFormat :PublishFormat, ..._) :Boolean {
            return exportFormat.modified;
        });
    }
    ...
    public function publishSingle (lib :XflLibrary) :int {
        ...
        for each (var format :PublishFormat in formats) {
            format.publish();
        }
    ...
    protected function instantiate (libs :Vector.<XflLibrary>,
        ...
        for each (var conf :ExportConf in _confs) {
            if (conf.combine && includeCombined) {
                ...
                formats.push(conf.createPublishFormat(_exportDir, new <XflLibrary>[libs[idx]], _projectName))
            }
        }
        return formats;
    }
